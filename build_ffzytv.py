...
def create_icon(text, size, bg, fg):
    ...
    # Use default font (Pillow handles it safely in CI)
...
""")
    ...
    # === CRITICAL: Write Gradle Wrapper files BEFORE keystore ===
...
""")
    ...
    # ✅ CORRECT gradlew (no extra quotes in JVM opts)
...
'''
    ...
    # gradlew.bat for Windows (optional but safe)
...
'''
    ...
    # === Generate keystore (non-fatal on failure) ===
    if not os.path.exists(keystore_path):
        ...
        try:
            ...
            print(f"⚠️  Keystore generation failed (stdout): {e.stdout}", file=sys.stderr)
            print(f"⚠️  Keystore generation failed (stderr): {e.stderr}", file=sys.stderr)
            print("⚠️  Release build may fail, but project structure is complete.", file=sys.stderr)
        except Exception as e:
            print(f"⚠️  Unexpected error during keystore generation: {e}", file=sys.stderr)
    else:
        print("ℹ️  Keystore already exists.")

    print(f"\n✅ Project '{PROJECT_NAME}' generated successfully!")
    print(f"📁 Project path: ./{PROJECT_NAME}/")
    print(f"🚀 Run: cd {PROJECT_NAME} && ./gradlew assembleRelease")
